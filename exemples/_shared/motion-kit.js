/* MOTION lab — runtime.
   Requer o gate `.motion` no <head> de cada página: sem ele, tudo fica no estado final. */
(function (w, d) {
  ...
  M.fit = function (p, a, b, from, to) {
    var t = Math.min(1, Math.max(0, (p - a) / (b - a)));
    return (from == null ? 0 : from) + ((to == null ? 1 : to) - (from == null ? 0 : from)) * t;
  };
  ...
  /* Divide texto em letras com aria-label no pai */
  ...
  M.scramble = function (el, text, dur) {
    ...
    (function tick(now) {
      ...
      for (var i = 0; i < text.length; i++) {
        out += text[i] === ' ' ? ' ' : p > i / text.length ? text[i] : G[(Math.random() * G.length) | 0];
      }
      el.textContent = out;
  ...
  M.spring = function (get, set, target, o) {
    o = o || {};
    var k = o.stiffness || 100, c = o.damping || 20, m = o.mass || 1, v = 0, last = performance.now(), run = true;
    (function step(now) {
      if (!run) return;
      var dt = Math.min(.032, (now - last) / 1000); last = now;
      var x = get(), a = (-k * (x - target) - c * v) / m;
      v += a * dt; x += v * dt; set(x);
      var rest = o.rest || .04;
  ...
  /* Loop rAF que pausa fora da viewport e com a aba oculta */
  ...
  M.particles = function (c, o) {
    if (!c) return;
    o = o || {};
    var g = c.getContext('2d'), dpr = Math.min(2, w.devicePixelRatio || 1);
    function size() { c.width = c.clientWidth * dpr; c.height = c.clientHeight * dpr; }
    size(); w.addEventListener('resize', size);
    if (!M.on()) return;
    var dots = [];
    for (var i = 0; i < (o.count || 60); i++) dots.push({ x: Math.random(), y: Math.random(), r: (.8 + Math.random() * 1.6) * dpr, v: .00015 + Math.random() * .00025 });
    M.loop(c, function () {
      g.clearRect(0, 0, c.width, c.height);
      g.fillStyle = o.color || 'rgba(220,80,0,.55)';
      dots.forEach(function (p) {
        p.y -= p.v; if (p.y < 0) p.y = 1;
        g.beginPath(); g.arc(p.x * c.width, p.y * c.height, p.r, 0, 6.2832); g.fill();
      });
    });
  };

  /* GSAP com fallback: se o CDN falhar ou demorar > 4s, remove .motion e a página fica estática e completa. */
  w.withGsap = function (run) {
    if (!M.on()) return;
    function load(src) {
      return new Promise(function (ok, no) {
        var s = d.createElement('script');
        s.src = src; s.async = false; s.onload = ok; s.onerror = no;
        d.head.appendChild(s);
      });
    }
    var b = 'https://cdn.jsdelivr.net/npm/gsap@3.12.7/dist/';
    var ready = w.gsap && w.ScrollTrigger ? Promise.resolve()
      : load(b + 'gsap.min.js').then(function () { return load(b + 'ScrollTrigger.min.js'); });
